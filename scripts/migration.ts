...
(async function insertJSON(json = {  // Inserire il JSON contenente gli user,master,score e questions
    "users": [
        {
            "id": 523350454,
            "is_bot": false,
    ...
    Object.keys(json).forEach(key => getModel(key).insertMany(json[key], function (err, users) {
        if (err) {
            return err;
        }
